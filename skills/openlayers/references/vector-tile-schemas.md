...
### 🌐 Consuming MapTiler Countries Schema in OpenLayers

In OpenLayers applications, developers consume the MapTiler Countries Schema through two primary architectural patterns:

#### Pattern 1: Native OpenLayers Vector Tile Layer (`ol/source/VectorTile` + `ol/format/MVT`)
Directly consume MapTiler `.pbf` tiles with zero external dependencies:
```javascript
...
const countriesVectorTileLayer = new VectorTileLayer({
  source: new VectorTileSource({
    format: new MVT(),
    url: `https://api.maptiler.com/tiles/countries/{z}/{x}/{y}.pbf?key=${KEY}`
  }),
  style: (feature) => {
    ...
    const density = demographicData[iso]?.density || 0;
    return new Style({
      fill: new Fill({
...
map.addLayer(countriesVectorTileLayer);

// Hit detection reading schema properties:
map.on('click', (evt) => {
  map.forEachFeatureAtPixel(evt.pixel, (feature, layer) => {
    if (layer === countriesVectorTileLayer) {
      const name = feature.get('name:en') || feature.get('name');
      const iso = feature.get('iso_a2');
      const area = feature.get('area');
      const continent = feature.get('continent');
      console.log(`Clicked ${name} (${iso}), Area: ${area} km², Continent: ${continent}`);
    }
  });
});
```

#### Pattern 2: Via Standard GeoJSON adhering to MapTiler Schema (`ol/layer/Vector`)
...
const vectorLayer = new VectorLayer({
  source: new VectorSource({
    url: '/data/countries-maptiler-schema.geojson',
    format: new GeoJSON()
  }),
  style: (feature) => {
    return new Style({
      fill: new Fill({
        color: getDensityColor(feature.get('density'))
      }),
      stroke: new Stroke({
        color: 'rgba(255, 255, 255, 0.4)',
        width: 1
